...
from trainer.trainer_utils import (get_lr, Logger, is_main_process, init_distributed_mode,
                                   ...
                                   get_device, get_device_type, empty_cache)
    ...
    for step, (input_ids, labels, pixel_values) in enumerate(loader, start=start_step + 1):
        ...
        if (step % args.save_interval == 0 or step == iters - 1) and is_main_process():
            ...
            vlm_checkpoint(vlm_config, weight=args.save_weight, model=model, optimizer=optimizer, 
                         epoch=epoch, step=step, wandb=wandb, save_dir='../checkpoints', scaler=scaler if use_scaler else None)
            model.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device", type=str, default=get_device(), help="训练设备")
    ...
    parser.add_argument('--keep_ratio', default=1.0, type=float, help="视觉token保留比例 (1.0=无剪枝, 0.5=保留50%)")
    parser.add_argument('--pruning_strategy', default='none', type=str, choices=['none', 'attention_pool', 'score_prune'],
                        help="剪枝策略: none/attention_pool/score_prune")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        loader = DataLoader(train_ds, batch_sampler=batch_sampler, num_workers=args.num_workers, pin_memory=pin_memory)
        if skip > 0: 
